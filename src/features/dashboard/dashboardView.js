  ...
  document.getElementById('turmas-grid').innerHTML = filtradas.map(t => {
    ...
    return `<div class="turma-card ${inativa ? 'turma-card-inativa' : ''}">
      ...
      <div style="font-size:12px;color:var(--text-3);display:flex;flex-direction:column;gap:6px;">
        ...
        <span>${ultimaAula ? `<span class="badge badge-blue" title="Última aula com presença registrada">📍 Até ${ultimaAula}</span>` : `<span class="badge badge-gray" title="Nenhuma presença registrada ainda">Sem aulas registradas</span>`}</span>
        <div>${horarioTag}</div>
      </div>
      ${inativa
