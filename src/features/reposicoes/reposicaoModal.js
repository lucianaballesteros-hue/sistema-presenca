...
// Dica visual: até qual aula essa turma já tem registro de presença — ajuda
// a equipe a achar uma turma que ainda vai passar pela aula que falta repor.
function aulaAtualDaTurma(turmaId) {
  let ultima = null;
  AULAS.forEach(au => {
    const key = `${turmaId}_${au}`;
...
export function renderOpcoesReposicao() {
  ...
  wrap.innerHTML = opcoesTemp.map((o, i) => {
    const dica = o.turmaDestinoId ? aulaAtualDaTurma(parseInt(o.turmaDestinoId)) : null;
    return `<div class="rep-opcao-row">
      <div class="rep-opcao-linha">
        <span class="rep-opcao-num">${i + 1}</span>
      ...
      <input type="text" class="rep-opcao-obs" placeholder="Observação para o aluno (opcional, ex: chegar 10 min antes)" value="${escapeAttr(o.observacao)}" onchange="atualizarOpcaoReposicao(${i},'observacao',this.value)"/>
...
export async function salvarReposicao() {
  ...
  if (error) { console.error(error); erroEl.textContent = 'Erro ao criar reposição. Tente novamente.'; return; }

  fecharModal('modal-reposicao');
  mostrarLinkReposicao(data.token, a.nome);
  renderReposicoes();
}

function mostrarLinkReposicao(token, nomeAluno) {
  const link = new URL(`reposicao.html?token=${token}`, window.location.href).href;
  document.getElementById('rep-link-desc').textContent = `Envie este link para ${nomeAluno} (ou responsável) marcar a reposição:`;
