...
| `reposicoesRepo.js` | `reposicoes` + `reposicao_opcoes` | `carregarReposicoes()` (duas consultas simples + junção no JS — de propósito não usa o "embedded select" do PostgREST `select('*, reposicao_opcoes(*)')`, que depende do cache de schema já reconhecer a FK entre as tabelas e pode falhar silenciosamente logo após a migração), `criarReposicao()`, `cancelarReposicao()`, `concluirReposicao()`, `buscarOpcaoReposicao()`, `assinarMudancasReposicoes()` (Realtime — ver 10.8). Usado só pelo painel autenticado; a página pública (seção 16) não usa este arquivo, fala com o Supabase por duas funções SQL separadas. |
...
- **`ultimaAulaRegistrada(turma)`**: última aula da turma que já tem algum registro de presença (olha só `state.PRESENCAS`, nenhuma consulta nova). Usado no cartão de turma do dashboard (10.3, badge "📍 Até Aula N") e na criação de reposição (10.8, dica de qual turma escolher).
...
- **`dashboardView.js`**:
  - `renderDash()`: desenha os 4 cartões de estatística (turmas, alunos ativos, irregulares, alertas), as "pills" de filtro por curso e a grade de cartões de turma — cada cartão mostra curso, professor, quantidade de alunos, quantos estão em alerta (clicável, ver `irParaAlertas`), um badge com `ultimaAulaRegistrada()` ("📍 Até Aula N", ou "Sem aulas registradas") para acompanhar o andamento da turma em tempo real, e o horário configurado.
  - A pill/atalho **"Turmas inativas"** só é desenhada e só fica navegável quando `state.perfilLogado?.papel === 'admin'` — professor não precisa (nem consegue pela interface) olhar turmas encerradas, nem as suas próprias. Se o estado guardar `filtroCurso === '__inativas__'` para um professor (ex.: sessão antiga), `renderDash()` reseta para `''` antes de desenhar.
...
### 10.8 `reposicoes/` — aba "Reposições" (equipe autenticada)

- **`reposicaoModal.js`**: `abrirModalReposicao()` (aberto a partir do modal de aluno) monta a lista de aulas com `calcAluno()`/`aulasDaTurma()` e pré-seleciona a primeira aula marcada como falta. A equipe monta até 4 opções de "turma destino + data" (`adicionarOpcaoReposicao`/`removerOpcaoReposicao`/`atualizarOpcaoReposicao`); `ultimaAulaRegistrada()` (domain/attendance.js, 8.1) dá a dica visual de até qual aula a turma escolhida já registrou presença. `salvarReposicao()` chama `criarReposicao()` e, ao terminar:
...
### 10.9 `metricas/metricasView.js` — aba "Métricas"
...
Coberto na [seção 10.8](#108-reposicoes--aba-reposições-equipe-autenticada): a equipe cria o caso a partir do modal de um aluno, gera o link, acompanha status (aberta/agendada/concluída/cancelada) na aba "Reposições", e ao confirmar que a reposição aconteceu, `concluirReposicaoAcao()` grava `'R'` na aula de origem — a mesma mecânica de "gravação" que já existia em `presencas` (ver 8.1), só que disparada por este fluxo em vez da tela de chamada.
